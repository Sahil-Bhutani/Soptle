...
const Register = () => {
    ...
    function getUser(){
        axios.get("http://localhost:3001/users").then((res)=>{
            setUser(res.data)
            setField({...field,id:res.data.length + 1})
    ...
    function postData(){
        axios.post("http://localhost:3001/users" , field).then((res)=>{
            setField({
                email: '',
                password: ''
              });
      ...
      <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                  ...
                  <button type="submit" className="w-full text-white bg-[#2563eb] hover:bg-[#1d4ed8] focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#2563eb] dark:hover:bg-[#1d4ed8] dark:focus:ring-primary-800">Create an account</button>
                  <h1 className='text-center'>OR</h1>
                  <p className='w-full text-white bg-[#2563eb] hover:bg-[#1d4ed8] focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#2563eb] dark:hover:bg-[#1d4ed8] dark:focus:ring-primary-800 cursor-pointer' onClick={()=>navigate("/loginadmin")}>Login Here</p>
              </form>
          </div>
      </div>
  </div>
